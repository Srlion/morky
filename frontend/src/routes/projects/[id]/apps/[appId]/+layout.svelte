<script>
    ...
    const currentTab = $derived(
        page.url.pathname.includes("/deployments")
            ...
            : page.url.pathname.includes("/logs")
              ? "logs"
              : page.url.pathname.includes("/volume")
                ? "volume"
                : "settings",
    );
    ...
    const tabs = [
        ...
        { id: "volume", path: "volume", label: "Volume" },
    ];
</script>
...
{#if app}
    ...
    >
        <div>
            ...
            {#if app.repo}
                <p class="text-sm text-base-content/60 mt-0.5 font-mono">
                    {app.repo}
                </p>
            {/if}
        </div>
    ...
    <div role="tablist" class="tabs tabs-bordered mb-6">
        {#each tabs as t}
            <a
                role="tab"
                href="{base}/{t.path}"
                class="tab {currentTab === t.id ? 'tab-active' : ''}"
                >{t.label}</a
            >
        {/each}
    </div>
